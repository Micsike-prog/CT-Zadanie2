...
def _decode_image(event: dict) -> Image.Image:
    if "image_base64" in event:
        ...
        body = event.get("body")
        if body is None:
            raise ValueError("Chýba telo požiadavky.")
    ...
    if not image_base64:
        raise ValueError("JSON požiadavky musí obsahovať pole 'image_base64'.")
...
def _prepare_predictions(raw_output: np.ndarray) -> np.ndarray:
    ...
    if predictions.ndim != 2:
        raise ValueError(f"Neočakávaný tvar výstupu ONNX modelu: {raw_output.shape}")
...
def _run_inference(image: Image.Image) -> list[dict]:
    ...
    if predictions.shape[1] < 5:
        raise ValueError(f"Neočakávaná šírka predikcie: {predictions.shape}")

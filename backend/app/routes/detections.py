...
async def read_limited_image(image: UploadFile) -> bytes:
    ...
    if len(image_bytes) > settings.max_upload_bytes:
        ...
        raise HTTPException(
            ...
            detail=f"Nahrávaný obrázok je príliš veľký. Maximálna veľkosť je {max_mb} MB.",
        )
    return image_bytes
        ...
        with Image.open(BytesIO(image_bytes)) as parsed_image:
            if parsed_image.format not in SUPPORTED_IMAGE_FORMATS:
                raise HTTPException(
                    ...
                    detail="Nepodporovaný formát obrázka. Použite JPEG, PNG alebo WebP.",
                )
            parsed_image.verify()
    except (OSError, UnidentifiedImageError) as exc:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Nahraný súbor nie je platný obrázok.",
        ) from exc
...
async def detect(
    ...
    if not image.content_type or not image.content_type.startswith("image/"):
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Nahraný súbor musí byť obrázok.")

    image_bytes = await read_limited_image(image)
    if not image_bytes:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Nahraný obrázok je prázdny.")
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=f"Neplatná odpoveď s detekciami z Lambda funkcie: {exc}",
        ) from exc

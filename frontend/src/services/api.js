...
/**
 * Pošle obrázok na backend a vráti ohraničujúce rámiky z YOLOv8.
...
/**
 * Načíta detail uloženej analýzy vrátane detekcií.
...
export async function fetchHistoryDetail(analysisId, token) {
  ...
  if (!response.ok) {
    throw new Error(response.status === 404 ? "Analýza sa nenašla." : `Chyba servera: ${response.status}`);
  }
